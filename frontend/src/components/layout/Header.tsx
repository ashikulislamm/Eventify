...
export default function Header() {
  ...
  const { user, club, userType, logout, initialize, initialized } = useAuthStore();
  ...
  const getDashboardLink = () => {
    ...
    } else if (userType === "club" && club) {
      return `/club/${club.club_name.toLowerCase().replace(/\s+/g, "")}`;
    }
    return "/";
  };
  ...
  const getDisplayName = () => {
    if (userType === "user" && user) {
      ...
      return club.nickname || club.club_name;
    }
    return "";
  };
